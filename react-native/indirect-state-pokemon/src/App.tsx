...
const PokemonList = () => {
  ...
  return (
    <ul className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 mt-3">
      {/* make the pokemoin cards clickable with <Link> */}
      {pokemon.map((p) => (
        <Link
          key={p.id}
          to={`/pokemon/${p.id}`}
...
function PokemonDetail() {
  ...
  // NO POKEMON FOUND, SRY!
  if (!pokemonData) {
    return <div>No pokemon found</div>;
  }

  return (
    // Show results of the Pokemon data in JSON format
    // <div>{JSON.stringify(pokemonData)}</div>

    // actual formatted HTML for Pokemon Details
    //
    // container div in TailwindCSS
    <div className="mt-3">
      <Link to="/">
          ...
          <div className="mt-3">
            ...
            <ul className="mt-3">
              {[
                ...
                "special_defense",
                "speed",
                // two column grid that loops through the Stats & Pokemon data
              ].map((stat) => (
...
function App() {
  //
  // queryClient context based provider
  return (
